...
**The full prioritised list, including the naming decision, now lives in
`OUTSTANDING-TASKS.md`.** That file is updated as things are done; this one would drift.

Top three, all live right now:

1. 🔴 `asci-vps-1` — **password SSH is enabled for root on a public IP** (three config files
   disagree, `50-cloud-init.conf` wins). The SSG hardening was overridden and never applied.
2. 🔴 `kami-vps-1` — **reboot pending** since 2026-09-15.
3. 🔴 `asci-vps-1` — **n8n on `0.0.0.0:5678`**, verify its owner account is set.
