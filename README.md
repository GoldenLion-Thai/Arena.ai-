...
app.html   ──▶ assets/js/app.js         workspace controller
              ...
              ├─ engine.js               streaming: on-device demo responder + real SSE client
...
npm test        # 96 runtime assertions across the three pages
npm start       # static server on 0.0.0.0:8080
```

`tests/smoke.mjs` loads each page in jsdom with a real IndexedDB and WebCrypto shim and drives the
actual UI: it sends a prompt, waits for streaming to finish, checks that TTFT and citations are
attached, stops a generation mid-stream and verifies the partial output survives, round-trips the
AES-GCM vault (including rejecting a wrong passphrase), exercises the model picker and the external
endpoint confirmation gate, and proves that retention 0 writes nothing to disk.
